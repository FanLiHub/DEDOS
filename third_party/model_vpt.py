...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompt_length=0, prompt_depth=0,patch_size=None):
        super().__init__()
        self.width = width
        self.layers = layers
        ...
        self.query_trans=[]
        self.query_trans=nn.ModuleList([nn.Linear(256, self.embed_dims) for i in range(layers)])
        # for ii in range(layers):
        #     self.query_trans.append(nn.Linear(256, self.embed_dims))
        for ii in range(layers):
            nn.init.kaiming_uniform_(self.query_trans[ii].weight, a=math.sqrt(5))


    def forward(self, x: torch.Tensor, dense=False, query=None, prompt=None):
        token_list=[]
        for i, resblock in enumerate(self.resblocks):
            ...
            if i == self.layers - 1 and dense:
                x = resblock.forward_dense(x, self.prompt_length)       ## eliminate the pooling effect
            else:
            ...
            if query !=None:
                ...
                tokens = self.query_trans[i](query)
                ...
                delta_feat = self.mlp_delta_f(delta_f + feats)
...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, prompt_depth: int, prompt_length: int):
        ...
        self.transformer = Transformer(width, layers, heads, prompt_depth=prompt_depth, prompt_length=prompt_length, patch_size=patch_size)
    ...
    def forward(self, x: torch.Tensor, dense=False, query_=None):
        ...
        x,token_list = self.transformer(x, dense, query_)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, query_= None, masks=None, pool_mask=None, dense=False):
        if pool_mask is not None:
            return self.visual(image.type(self.dtype), mask=pool_mask, dense=dense)
        if query_ is not None:
            return self.visual(image.type(self.dtype), dense=dense, query_=query_)
        elif masks == None:
