...
class ClassTransformerLayer(nn.Module):
    def __init__(self, hidden_dim=256, guidance_dim=256, nheads=4, attention_type='linear', pooling_size=(4, 4), pad_len=256) -> None:
        super().__init__()
    ...
    def forward(self, x, guidance):
        ...
        if self.padding_tokens is not None:
            ...
            if orig_len < self.pad_len:
                # pad to pad_len
                padding_tokens = repeat(self.padding_tokens, '1 1 C -> B T C ', B=x.shape[0], T=self.pad_len - orig_len)
                x_pool = torch.cat([x, padding_tokens], dim=1)

        ## guidance [1,171,128]
        if guidance is not None:
            if self.padding_guidance is not None:
                if orig_len < self.pad_len:
                    padding_guidance = repeat(self.padding_guidance, '1 1 C -> 1 T C', T=self.pad_len - guidance.shape[1])
                    guidance = torch.cat([guidance, padding_guidance], dim=1)
            guidance = repeat(guidance, '1 T C -> B T C', B=100)
...
class Aggregator(nn.Module):
    ...
    def forward(self, img_feats, text_feats, appearance_guidance):
        ...
        corr = self.correlation(img_feats, text_feats)      ## get cost volume
        ...
        for layer in self.layers:   ## spatial agg & class agg
            corr_embed = layer(corr_embed, projected_guidance, projected_text_guidance)

        logit = self.conv_decoder(corr_embed, projected_decoder_guidance)   ## upsampling & head
